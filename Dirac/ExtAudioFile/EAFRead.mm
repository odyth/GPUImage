...
- (OSStatus) openFileForRead:(NSURL*)fileURL sr:(Float64)sampleRate channels:(int)numChannels
{
...
#endif
	{
		...
		if (err) {NSLog(@"!!! Error in ExtAudioFileOpen, %d", (int)err); return err;}
		...
		if (err) {NSLog(@"!!! Error in ExtAudioFileGetProperty, %d", (int)err); return err;}
...
- (OSStatus) readSInt16ConsecutiveFromAsset:(UInt32*)numPackets intoArray:(SInt16*)buffer
{
		...
		if (buffer) {			
			...
			for (long v = 0; v < numSamplesToRead; v++) {
				...
				if (mAssetNumSamplesRead++ >= mAssetNumSamplesInBuffer-1) {
					...
					err = (int)[self pullNewSamplesFromAsset:mAssetReaderOutput];
	...
	if (err >= 0)
		*numPackets = (int)samplesObtained / mExtAFNumChannels;
	else {
...
-(SInt16*)copyFrames:(SInt64*)numFrames error:(OSStatus*)err
{
	*err = noErr;
	...
	UInt32 numPackets = (int)*numFrames; // Frames to read
...
-(SInt64)fileNumFrames
{
...
#endif
	{
		...
		@synchronized (self) {
			...
			if (err) {NSLog(@"!!! Error in ExtAudioFileGetProperty kExtAudioFileProperty_FileLengthFrames, %d", (int)err);}
		}
